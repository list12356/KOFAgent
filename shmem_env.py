...
def make_env(env_id, device):
    def _thunk():
        env = Environment(env_id, roms_path, frame_ratio=2, frames_per_step=3, difficulty=3)
...
class SFEnvironmentShmem(VecEnv):
    """
    Optimized version of SubprocVecEnv that uses shared variables to communicate observations.
    """

    def __init__(self, num_process, device, context='spawn'):
        """
    ...
    def step_wait(self):
        ...
        obs, rews, dones, _ = zip(*outs)
        rews = torch.FloatTensor([[r['P1'] for r in rews]])
        info = {}
        if round_done:
    ...
    def _decode_obses(self, obs):
        ...
        for k in self.obs_keys:
            ...
            result[k] = np.array(o)
        return transform_frames(dict_to_obs(result), self.device)
    ...
    try:
        while True:
            ...
            if cmd == 'reset':
                ...
                obs, reward, done, round_done, stage_done, game_done = env.step(data)
                if done:
                    obs = env.reset()
